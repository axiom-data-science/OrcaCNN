...
def train(model, img_width, img_height, train_data_path, validation_data_path):
    ...
    model.save('orca_podclassification.h5')

    logger.info("Pod Classification Model saved")


class OrcaNet:
    @staticmethod
    def build(img_width, img_height):
        '''Build CNN for Orca Pod-classification'''
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Train CNN model for classification of 20 orca pods")
    parser.add_argument(
        '-c',
        '--classpath',
        type=str,
        help='directories with 20 pod-classes',
        required=True)
